...
export const startBuild = async (build: Build, resource?: K8sResourceKind): Promise<BuildRun> => {
  const newBuildRunData: BuildRun = {
    ...
    metadata: {
      ...
      labels: {
        ...
        ...(resource
          ? {
              [BUILDRUN_TO_RESOURCE_MAP_LABEL]:
...
export const rerunBuildRun = async (buildRun: BuildRun): Promise<BuildRun> => {
  ...
  const resourceMapLabel = buildRun.metadata?.labels?.[BUILDRUN_TO_RESOURCE_MAP_LABEL];
